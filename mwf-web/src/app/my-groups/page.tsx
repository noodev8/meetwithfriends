        ...
        <SidebarLayout>
            <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-5xl mx-auto w-full">
                ...
                {loadingData ? (
                    ...
                    <>
                        {/* Groups You Run (Organiser or Host, but not "all members host" groups) */}
                        {groups.filter(g => g.role === 'organiser' || (g.role === 'host' && !g.all_members_host)).length > 0 && (
                            <section className="mb-10">
                                <h2 className="text-lg font-semibold text-slate-800 mb-4">Groups You Run</h2>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {groups
                                        .filter(g => g.role === 'organiser' || (g.role === 'host' && !g.all_members_host))
                                        .map((group) => (
                        ...
                        {/* Groups You're In (Member or "all members host" hosts) */}
                        {groups.filter(g => g.role === 'member' || (g.role === 'host' && g.all_members_host)).length > 0 && (
                            <section className="mb-10">
                                <h2 className="text-lg font-semibold text-slate-800 mb-4">Groups You&apos;re In</h2>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {groups
                                        .filter(g => g.role === 'member' || (g.role === 'host' && g.all_members_host))
                                        .map((group) => (
                                            <GroupCard key={group.id} group={group} />
                                        ))}
                                </div>
