...
class _EventDetailScreenState extends State<EventDetailScreen> {
  ...
  Widget _buildStatusRow(EventDetail event) {
    return Wrap(
      ...
      children: [
        ...
        // Export button (for hosts/organisers)
        if (_canEdit)
          GestureDetector(
            onTap: () => _showExportBottomSheet(event),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
              decoration: BoxDecoration(
                color: const Color(0xFFF8FAFC),
  ...
  void _showExportBottomSheet(EventDetail event) {
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () {
                  ...
                  _copyGuestList(event);
                },
              ),

              // Download Pre-Orders PDF option (only if pre-orders enabled)
  ...
  Future<void> _copyGuestList(EventDetail event) async {
    // Fetch full attendee list
    final result = await _eventsService.getAttendees(event.id);

    if (!result.success || result.attending.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No attendees to copy'),
    ...
    // Build guest list text
    final buffer = StringBuffer();
    buffer.writeln('${event.title} - Guest List');
    buffer.writeln('${result.attending.length} attending');
    buffer.writeln();

    for (final attendee in result.attending) {
      String line = attendee.name;
      if (attendee.guestCount > 0) {
        line += ' (+${attendee.guestCount} guest${attendee.guestCount > 1 ? 's' : ''})';
      }
      buffer.writeln(line);
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Guest list copied to clipboard'),
          backgroundColor: Color(0xFF22C55E),
          duration: Duration(seconds: 2),
        ),
      );
  ...
  Future<void> _downloadPreOrdersPDF(EventDetail event) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiService.baseUrl}/events/${event.id}/preorders/pdf'),
        headers: {
      ...
      final fileName = 'preorders-${event.title.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]+'), '-')}.pdf';
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        subject: '${event.title} - Pre-Orders',
      );
    } catch (e) {
